...
사용 가능한 스킬 목록에서 `format-and-lint` 스킬이 있는지 확인한다:
- **있으면** → Skill 도구로 `format-and-lint`을 호출한다. 실행 후 새로운 변경사항이 생겼다면 다음 스테이징 단계에서 함께 포함된다.
- **없으면** → "`setup-format-lint` 스킬을 사용하면 이 프로젝트에 포맷/린트 스킬을 생성할 수 있습니다." 안내를 출력하고 다음 단계로 진행한다
