...
이름이 짧거나 일반적인 경우(예: "go", "node") 매칭 결과가 많을 수 있다. 이 경우 `pgrep -xi "<name>"`(프로세스 이름 정확 일치)을 먼저 시도하고, 결과가 없으면 `-fi`로 확대한다.

- **실행 중이 아닌 경우** → 3단계로 진행
- **실행 중인 경우** → 매칭된 프로세스 목록(`ps -p <pid> -o pid,comm,args`)을 `AskUserQuestion`에 포함하여 확인:
...
mdfind가 결과를 반환하지 않거나 에러가 발생한 경우, Spotlight 인덱싱이 비활성화되었을 수 있다는 안내를 출력하고 나머지 탐색 결과로 계속 진행한다.

#### 3-4. 패키지 매니저 경로

설치 소스에 따라 해당 경로도 포함한다:
- Homebrew formula: `brew --prefix <name>` 하위 경로
- Homebrew cask: `brew --caskroom`/<name> 하위 경로
- npm: `npm root -g` 하위 경로
- cargo: `~/.cargo/bin/`
- pip: `pip show <name>`의 Location 경로
- pipx: `~/.local/pipx/venvs/<name>`
- go: `~/go/bin/`
- gem: `gem environment gemdir`/gems/<name>*
...
- mas: `mas uninstall`이 지원되지 않는 버전의 경우, .app 번들을 직접 삭제한다

각 삭제 명령이 실패하면 에러를 기록하고 다음 항목으로 계속 진행한다. 실패 항목은 6단계에서 보고한다.

#### 잔여 파일 — 휴지통으로 이동

삭제 전 파일 존재 여부를 확인한다 (패키지 매니저 제거로 이미 삭제되었을 수 있다).
